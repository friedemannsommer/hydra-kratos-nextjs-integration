...
export default function Login(): ReactElement {
    ...
    }, [createFlow, getLoginFlow, refresh, aal, returnTo, flowId, router.isReady, flow])
    ...
    const submitFlow = useCallback(
        (values: UpdateLoginFlowBody) => {
            ...
            router.push(`?flow=${flow.id}`, undefined, { shallow: true }).catch(console.error)
            ...
            kratos
                ...
                .catch((err: AxiosError) => {
                    switch (err.response?.status) {
                        ...
                        case 422: {
                            const flow = new URL(
                                (err.response.data as Record<string, string>).redirect_browser_to,
            ...
            {flow ? (
                <>
                    <UserAuthCard
                        ...
                        includeScripts={true}
                        onSubmit={({ body }): void => submitFlow(body as UpdateLoginFlowBody)}
                    />
